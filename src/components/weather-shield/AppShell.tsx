          ...
          <div className="p-3 flex flex-col gap-3 [&>*]:shrink-0">
            ...
            {(journey.state === 'IDLE' || journey.state === 'ANALYZING' || !journey.routes || journey.routes.length === 0) ? (
              ...
              <>
                ...
                <div className="flex flex-col p-2 rounded-xl bg-slate-800/50 border border-slate-700/50 cursor-pointer hover:bg-slate-800/70 transition-colors" onClick={() => dispatch({ type: 'SET_STATE', payload: 'IDLE' })}>
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      <span className="text-slate-400 font-medium">From</span>
                      <span className="font-semibold text-slate-200 truncate max-w-[90px]">{journey.origin.name}</span>
                      <span className="text-slate-500">→</span>
                      <span className="font-semibold text-slate-200 truncate max-w-[90px]">{journey.destination.name}</span>
                    </div>
                    <span className="text-[9px] text-indigo-400 uppercase tracking-wider bg-indigo-900/30 px-1.5 py-0.5 rounded border border-indigo-500/30">Edit</span>
                  </div>
                </div>
                ...
                <div className={cn("transition-opacity duration-500 flex flex-col gap-1.5 [&>*]:shrink-0", journey.state === 'EMERGENCY_MODE' ? "opacity-50 pointer-events-none" : "opacity-100")}>
                  <div className="bg-slate-900/80 rounded-xl p-2 border border-slate-700/50 shadow-md">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Route Forecast</h3>
